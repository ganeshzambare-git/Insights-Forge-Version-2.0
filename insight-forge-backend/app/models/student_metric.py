...
class StudentMetric(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "success_indicator_status IN " "('Safe', 'Amber', 'Critical')",
            name="success_indicator_status",
        ),
        CheckConstraint(
            "attendance_percentage >= 0 " "AND attendance_percentage <= 100.00",
            name="attendance_percentage_range",
        ),
        CheckConstraint(
            "raw_average_grade >= 0 " "AND raw_average_grade <= 100.00",
